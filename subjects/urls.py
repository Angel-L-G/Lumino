...
urlpatterns = [
    ...
    re_path(
        r'(?P<code>[A-Z]{3})/lessons/(?P<pk>\d+)/delete/', views.delete_lesson, name='delete-lesson'
    ),
    re_path(r'(?P<code>[A-Z]{3})/lessons/(?P<pk>\d+)/edit/', views.edit_lesson, name='edit-lesson'),
    re_path(r'(?P<code>[A-Z]{3})/lessons/(?P<pk>\d+)/', views.lesson_detail, name='lesson-detail'),
